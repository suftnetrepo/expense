...
import { validate, StyledSpinner, XStack, YStack, StyledBadge, StyledText, StyledCheckBox, StyledCard, StyledCycle, StyledDialog, StyledOkDialog, StyledHeader, StyledSafeAreaView, StyledSpacer, StyledButton } from 'fluent-styles';
import { fontStyles, theme } from "../../configs/theme";
import { useNavigation, useRoute } from "@react-navigation/native";
import DateTimePicker from '@react-native-community/datetimepicker';
import { KeyboardAwareScrollView } from "react-native-keyboard-aware-scroll-view";
import { cardRules } from "./validatorRules";
import { useInsertCard } from "../../hooks/useCard";
...
const AddCard = () => {
  ...
  const { insert, error, loading, resetHandler } = useInsertCard()
  ...
  const onSubmit = async () => {
    ...
    const handleResult = () => {
      ShowToast("Success", "Card was added successfully", "success")
      setFields(cardRules.reset)
    }

    await insert({ ...fields, child_id: child.child_id }).then(async (result) => {
      result && handleResult()
    })
  }
  ...
  return (
    <StyledSafeAreaView backgroundColor={theme.colors.gray[1]}>
      <StyledHeader marginHorizontal={8} statusProps={{ translucent: true }} >
        ...
        })} title='Schedule Vaccine' icon cycleProps={{
